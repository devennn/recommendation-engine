...
def cost(X, *args):
    ...
    :param X: The matrices being factored (P and Q) rolled up
    ...
    a = np.sum(np.square(mask * (np.dot(P, Q) - ratings))) / 2
    b = (regularization_amount / 2.0) * np.sum(np.square(Q.T))
    c = (regularization_amount / 2.0) * np.sum(np.square(P))
    return (a + b + c)
...
def gradient(X, *args):
    ...
    a = np.dot((mask * (np.dot(P, Q) - ratings)), Q.T)
    P_grad = a + (regularization_amount * P)
    b = np.dot((mask * (np.dot(P, Q) - ratings)).T, P)
    Q_grad = b + (regularization_amount * Q.T)

    # Return the gradients as one rolled-up array as expected by fmin_cg
    return np.append(P_grad.ravel(), Q_grad.ravel())


def lrank_mtrx_fctrize(ratings, mask=None, num_features=15,
                       regularization_amount=0.01):
    """
    Factor a ratings array into two latent feature arrays
    (user features and product features)

    :param ratings: Matrix with user ratings to factor
    :param mask: A binary mask of which ratings are present in
     the ratings array to factor
    ...
    # If no mask is provided, consider all 'NaN' elements as
    # missing and create a mask.
    if mask is None:
    ...
    # Call fmin_cg to minimize the cost function and this find
    # the best values for P and Q
    X = fmin_cg(cost, initial, fprime=gradient, args=args, maxiter=3000)

    # Unroll the new P and new Q arrays out of the contiguous
    # array returned by fmin_cg
...
def RMSE(real, predicted):
    """
    Calculate the root mean squared error between a matrix of
    real ratings and predicted ratings
    :param real: A matrix containing the real ratings
    (with 'NaN' for any missing elements)
